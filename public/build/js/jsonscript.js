...
$(document).ready(function () {
	...
	if ($('#companieslist').length > 0) {
		var companiesTable = $('#companieslist').DataTable({
			"bFilter": true, // Ensure search is enabled
			"bInfo": false,
			"ordering": true,
			"autoWidth": true,
			"language": {
				search: ' ', // Customize search label if needed
				sLengthMenu: '_MENU_',
			...
			"ajax": {
				url: '/companies/data', // Update URL to fetch client data
				type: 'GET',
				dataSrc: 'data', // The JSON response should have a 'data' key
			},
			"columns": [
				// Column for Company Name, Logo, and Address
				{
					"render": function (data, type, row) {
						var imageUrl = '/storage/' + row['company_logo']; // Assuming row['company_image'] is a relative path from public/storage
						return '<h2 class="d-flex align-items-center"><a href="company-details" class="avatar avatar-sm border rounded p-1 me-2"><img src="' + imageUrl + '" alt="User Image"></a><a href="company-details" class="d-flex flex-column">' + row['company_name'] + '<span class="text-default">' + row['address'] + ' </span></a></h2>';
					}
				},
				// Column for Phone
				{ "data": "phone" },
				...
				{
					"render": function (data, type, row) {
						return '<span class="title-name">' + (row['owner'] || 'N/A') + '</span>';
					}
				},
				// Column for Created Date
				{ "data": "created_date" },
				// Column for Status (Active/Inactive)
				{
					"render": function (data, type, row) {
				...
				// Column for Actions (Edit/Delete)
				{
					"render": function (data, type, row) {
						return '<div class="dropdown table-action">' +
							...
							'<a class="dropdown-item" data-bs-toggle="offcanvas" data-bs-target="#offcanvas_edit_company" ' +
							'data-client-id="' + (row['id'] || '') + '" ' +
							'data-company-name="' + (row['company_name'] || '') + '" ' +
							'data-company-logo="' + (row['company_logo'] || '') + '" ' +
							'data-street-address="' + (row['address'] || '') + '" ' +
							...
							'data-created-date="' + (row['created_date'] || '') + '" ' +
							'data-city="' + (row['city'] || '') + '" ' +
							'data-state-province="' + (row['state_province'] || '') + '" ' +
							...
							'</a>' +
							'<a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#delete_company" ' +
							'data-client-id="' + row['id'] + '">' +
		...
		// Bind custom search functionality to the search input field
	...
	if ($('#leads_list').length > 0) {
		var leadsTable = $('#leads_list').DataTable({
			"bFilter": true, // Ensure search is enabled
			"bInfo": false,
			"ordering": true,
			"autoWidth": true,
			...
			"ajax": {
				url: '/leads/data', // Replace this URL with your actual route to fetch data
				type: 'GET',
				dataSrc: 'data', // The JSON response should have a 'data' key
			},
			"columns": [
				...
				{
					"render": function (data, type, row) {
						var imageUrl = '/storage/' + row['company_image']; // Assuming row['company_image'] is a relative path from public/storage
						return '<h2 class="d-flex align-items-center"><a href="company-details" class="avatar avatar-sm border rounded p-1 me-2"><img src="' + imageUrl + '" alt="User Image"></a><a href="company-details" class="d-flex flex-column">' + row['company_name'] + '<span class="text-default">' + row['company_address'] + ' </span></a></h2>';
					}
				},
				{ "data": "phone" },
				{ "data": "email" },
				...
				},
				{
					"render": function (data, type, row) {
						return '<div class="dropdown table-action">' +
							...
							'</a>' +
							'<div class="dropdown-menu dropdown-menu-right">' +
							'<a class="dropdown-item" data-bs-toggle="offcanvas" data-bs-target="#offcanvas_edit_lead" ' +
							'data-lead-id="' + (row['id'] || '') + '" ' +
							'data-company-id="' + (row['company_id'] || '') + '" ' +
							'data-lead-name="' + (row['lead_name'] || '') + '" ' +
							...
							'<i class="ti ti-edit text-blue"></i> Edit' +
							'</a>' +
							'<a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#delete_lead" ' +
							'data-lead-id="' + row['id'] + '">' +
							'<i class="ti ti-trash text-danger"></i> Delete' +
							'</a>' +
							'</div>' +
